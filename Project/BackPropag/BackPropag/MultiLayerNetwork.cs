...
namespace BackPropag
{
    ...
    public class MultiLayerNetwork
    {
        ...
        public int[] Layer_Definition { get { return layer_definition; } set { layer_definition = value;
                ...
                for (int l = 0; l < layer_definition.Length; l++)
                {
                    ...
                    for (int j = 0; j < layer_definition[l]; j++)
                    {
                        if (l == 0)
                        {
                            ...
                            Console.WriteLine("Adding Neuron {0} of dimension {1} in layer {2} {3}", j, this.dimension, l, this.NeuralNetwork[l][j].InputNeuron);
                        ...
                        else
                        {
                            ...
                            Console.WriteLine("Adding Neuron {0} of dimension {1} in layer {2} {3}", j, this.layer_definition[l - 1], l, this.NeuralNetwork[l][j].InputNeuron);
                        }
                    }
                }
            } }
        ...
        public void Train(List<double[]> pointlist, List<List<double>> expected, int cycles)
        {
            for (int c = 0; c < cycles; c++)
            {
                ...
                for (int idx = 0; idx < pointlist.Count; idx++)
                {
                    ...
                    Console.WriteLine("Initial input vector {0} ({1})", String.Join(";",this.inputs),this.inputs.Length);
                    ...
                    for (int layeridx = this.NeuralNetwork.Count-1; layeridx >= 0; layeridx--)
                    {
                        for (int neuridx = 0; neuridx < this.NeuralNetwork[layeridx].Count; neuridx++)
                        {
                            ...
                            else
                            {
                                ...
                                //neuroerror = (0.5 * Math.Pow((expected[idx][neuridx] - this.NeuralNetwork[layeridx][neuridx].output), 2));
                ...
                Console.WriteLine("----");
                if (Math.Round(Sum(this.errors),3) == 0)
                {
